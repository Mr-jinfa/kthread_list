/*程序设计:用kthread实现生产者消费者模型
*用klist来运载消息节点(类似消息队列)
*
*Author:黄进发
*Blogs:https://blog.csdn.net/huang_165
*
*Bug Report: 2112201069@qq.com
*GitHub: https://github.com/Mr-jinfa
*/
...
static int work_thread(void *data)
{
	...
	while(1){
		if(list_empty(&work_list))
			wait_for_completion(&work_wait);	//没有数据,进入work_wait.wait里休眠.
		while(list_empty(&work_list) !=1){
/* work_list 不为空时*/
			...
			if(event != NULL) {
				...
				if(event->type == EVENT_TIMER){
					printk(KERN_INFO "event timer: count = %d ", ++count);
					if(event->need_free){
						kfree(event);
...
MODULE_DESCRIPTION("Consumer & producer kthread model");
